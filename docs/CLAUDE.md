...
58. **編集フォームの入力欄が「propからローカル状態を初期化し、以降はコンポーネント自身の編集操作でのみ更新する」形になっている場合、そのpropの値がコンポーネント自身の編集以外の経路（ドラッグ操作、依存関係の自動カスケード、AI再生成、他ユーザーの操作等）で外部から変化しても、同じid基準のkeyでは再マウントされず、画面上に古い値が残り続けたまま次の保存で上書きしてしまう。** 実際に進捗画面で、ドラッグによる日付変更直後に同じ項目の詳細パネルを開いて別フィールドを編集すると、ドラッグ結果ではなく古い日付が`<input>`に表示されたまま保存されてしまう不具合が発生した。対処は、`key`にid以外の値（実際に外部から変化しうる値そのもの）も含めることで、値が変わった際に確実に再マウントさせること。「propから初期化するローカル編集state」というこのアプリ全体で多用しているパターンは、外部要因で値が変わりうるフィールド（ドラッグ・カスケード処理・AI生成・共同編集が絡む箇所等）では同様のリスクを抱えていないか、都度意識すること。
59. **楽観的更新（操作確定直後に画面をすぐ更新し、裏でサーバーへ保存する方式）を実装する際、操作確定と同時にローカルの上書き値を消してしまうと、サーバー側の再検証データが実際に届くまでの間、propに残る古い値が一瞬表示され「戻ってから進む」ちらつきが発生する。** 実際に進捗画面のドラッグ操作でこの現象が発生した。対処は、ローカルの上書き値を操作確定時点で即座に反映しつつ、サーバーからの実データが実際にその値に追いついたことを確認できるまで消さない設計（propの値と上書き値を比較し一致した時点で自然に無効化する）にすること。この消去判定を`useEffect`＋`setState`で行うと規約54のリント制約に抵触するため、レンダー計算の中で吸収すること。**さらに、並べ替えの楽観的更新では、各データの`order_index`プロパティを更新するだけでは不十分な場合がある。** 一覧の描画側が配列の格納順（`.map()`の順序）をそのまま使っている実装だと、`order_index`の値だけ変えても見た目の並びは変わらない。実際に非機能要件画面でこの不具合が発生し、一覧を描画する直前に必ず`order_index`で明示的にソートする（配列の格納順に依存しない）ことで解消した。
